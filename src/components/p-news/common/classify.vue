<template>
  <div class="news-classify">
    <div class="f-clearfix">
      <router-link v-if="lang === 'cn'" to="/admin/news/classifyadd" class="f-right button list-btn-add">
        <span class="icon icon-round_add"></span>添加
      </router-link>
      <router-link v-if="lang === 'en'" to="/admin/news/classifyadden" class="f-right button list-btn-add">
  ...
  export default {
    ...
    methods: {
      ...
      confirmPop() {
        ...
        this.pop.show = false
        this.axios(this.api.newsClassify.delete(deleteIds)).then((res) => {
          let data = res.data
          if (data.code === '200') {
