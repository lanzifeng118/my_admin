<template>
  <div class="info-list">
    <div class="f-clearfix">
      ...
      <router-link to="/admin/aboutus/adden" class="f-right button list-btn-add">
      ...
      <table v-if="items.length > 0">
        ...
        <tbody>
          <tr v-for="(item, index) in items">
            ...
            <td class="link">
              <router-link :to="'/admin/aboutus/editen/' + item.id">编辑</router-link>
  ...
  export default {
    ...
    methods: {
      ...
      toggleDisplay(index) {
        ...
        // ajax
        this.axios(api.aboutus.updateForDisplay({id: item.id, display: item.display})).then((res) => {
          let data = res.data
          console.log(data)
          if (data.code === '200') {
