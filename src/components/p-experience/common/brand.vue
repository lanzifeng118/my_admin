<template>
  <div class="experience-brand">
    <div class="f-clearfix">
      <router-link v-if="lang === 'cn'" to="/admin/experience/brandadd" class="f-right button list-btn-add">
        <span class="icon icon-round_add"></span>添加
      </router-link >
      <router-link v-if="lang === 'en'" to="/admin/experience/brandadden" class="f-right button list-btn-add">
      ...
      <table v-if="items.length > 0">
        <thead>
          <tr>
            ...
            <th>{{lang === 'cn' ? '品牌名称' : 'Brand Name'}}</th>
            <th width="400">{{lang === 'cn' ? '修改时间' : 'Edit Time'}}</th>
        ...
        <tbody>
          <tr v-for="(item, index) in items">
            ...
            <td class="link">
              <router-link v-if="lang === 'cn'" :to="'/admin/experience/brandedit/' + item.id">编辑</router-link>
              <router-link v-else :to="'/admin/experience/brandediten/' + item.id">Edit</router-link>
  ...
  export default {
    ...
    methods: {
      ...
      confirmPop() {
        ...
        this.pop.show = false
        this.axios(this.api.experienceBrand.delete(deleteIds)).then((res) => {
          let data = res.data
          if (data.code === '200') {
